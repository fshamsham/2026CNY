...
const RankItem: React.FC<{ 
  ...
  <div 
    onClick={onClick}
    className="group flex flex-col sm:flex-row items-start sm:items-center gap-5 md:gap-10 py-5 md:py-7 transition-all duration-300 hover:bg-red-50/30 cursor-pointer px-5 md:px-12"
  >
    {/* Thumbnail with Rank Overlay - Now full width on mobile */}
    <div className="relative w-full sm:w-48 md:w-64 lg:w-72 aspect-video rounded-[1.2rem] md:rounded-[2rem] overflow-hidden shrink-0 border border-gray-100 shadow-md">
      ...
      {/* Rank Badge */}
      <div className={`absolute top-2.5 left-2.5 w-8 h-8 md:w-11 md:h-11 flex items-center justify-center rounded-lg md:rounded-xl font-black text-xs md:text-base shadow-xl backdrop-blur-md border border-white/20
    ...
    <div className="flex-1 min-w-0 w-full">
      <h4 className="font-black text-red-950 text-base md:text-xl lg:text-2xl line-clamp-2 group-hover:text-red-600 transition-colors leading-tight mb-1.5 md:mb-3">
        {video.VideoTitle}
      </h4>
      
      <div className="flex items-center gap-2 mb-3 md:mb-4">
        <div className="w-4 h-4 md:w-5 md:h-5 rounded-md overflow-hidden border border-red-100 shrink-0 shadow-sm">
          <img 
        ...
        <p className="text-[10px] md:text-[11px] text-red-900/40 uppercase font-black tracking-[0.15em] md:tracking-[0.2em] truncate">
          {video.ChannelName}
        </p>
      </div>
      
      <div className="flex flex-col md:flex-row md:items-center gap-2 md:gap-5">
        <div className="flex items-baseline gap-1.5 bg-red-50/50 self-start px-2.5 py-1 rounded-lg md:bg-transparent md:p-0">
          <span className="text-lg md:text-2xl font-black text-red-600 tabular-nums leading-none tracking-tighter">
            {metricValue}
          </span>
          {metricLabel && (
            <span className="text-[9px] md:text-xs font-bold text-red-900/60 uppercase tracking-tight">
              {metricLabel}
            </span>
          )}
        </div>
        ...
        <div className="flex items-center gap-1.5 opacity-40">
          <Eye size={10} className="md:size-3.5 text-red-600" />
          <span className="text-[9px] md:text-[12px] text-red-950 font-black uppercase tracking-widest">
  ...
  return (
    <div className="space-y-8 md:space-y-12 w-full mx-auto">
      <div className="flex flex-col items-center text-center">
        ...
        <h2 className="text-3xl md:text-5xl lg:text-6xl font-black text-red-950 font-cny tracking-tight px-4 mb-8 md:mb-10">
          {t.rankingSectionTitle}
        </h2>

        {/* Filter Toggle Buttons */}
        <div className="flex flex-wrap justify-center gap-2 md:gap-3 px-4 w-full mb-8">
          {filters.map((filter) => (
            <button
              key={filter.id}
              onClick={() => setActiveFilter(filter.id as FilterType)}
              className={`flex items-center gap-2 px-4 py-2.5 md:px-6 md:py-3.5 rounded-full text-[10px] md:text-sm font-black uppercase tracking-wider transition-all duration-300 border ${
        ...
        <div className="w-full max-w-4xl px-6 mb-8 animate-in fade-in slide-in-from-top-2 duration-500">
          <div className="bg-red-50/50 rounded-2xl md:rounded-full px-6 py-4 md:py-5 border border-red-100">
            <p className="text-[11px] md:text-[14px] text-red-950/70 font-medium text-center leading-relaxed">
        ...
        <div className="flex items-center bg-gray-100/50 p-1 rounded-full border border-gray-100">
          <button 
            onClick={() => setLimit(5)}
            className={`px-5 py-1.5 rounded-full text-[9px] md:text-[10px] font-black uppercase tracking-widest transition-all ${limit === 5 ? 'bg-white text-red-600 shadow-sm' : 'text-gray-400 hover:text-red-900'}`}
          >
            Top 5
          </button>
          <button 
            onClick={() => setLimit(10)}
            className={`px-5 py-1.5 rounded-full text-[9px] md:text-[10px] font-black uppercase tracking-widest transition-all ${limit === 10 ? 'bg-white text-red-600 shadow-sm' : 'text-gray-400 hover:text-red-900'}`}
          >
            Top 10
          </button>
        </div>
      ...
      <div className="bg-white rounded-[2rem] md:rounded-[3rem] border border-gray-100 shadow-2xl shadow-red-900/5 overflow-hidden divide-y divide-gray-100">
